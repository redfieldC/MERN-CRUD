...
const CreatePage = () => {
    ...
    const saveProduct = async(e) => {
        ...
        try{
            setIsLoading(true);
            const response = await axios.post(`http://localhost:1000/api/products/create`, {name: name, quantity: quantity, price: price, image: image});
            toast.success(`Save ${name} sucessfully`)
            setIsLoading(false);
            navigate("/");
        }catch (error){
